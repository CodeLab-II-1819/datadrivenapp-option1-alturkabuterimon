#include <iostream>
#include <fstream>
#include <string>
#include <algorithm> //For toUpper/toLower
using namespace std;



//Functions

//Function to count tweets
int countTweets(ifstream &inFile, int numOfTweets, int offset, string toUpper, string toLower, string line)
{
	inFile.open("sampleTweets.csv");
	if (inFile.good())
	{
		cout << "Reading from file." << endl;
		while (!inFile.eof())
		{
			while (getline(inFile, line))
			{
				if (((offset = line.find(" " + toLower + " ", 0)) != string::npos) || ((offset = line.find(" " + toUpper + " ", 0)) != string::npos))  //Place a space before and after the word to stop any potential words within words
					//This line finds the word is on the line and saves that line to the variable line, it finds both upper and lowercase values.
				{
					numOfTweets++;
					cout << "Here is a line." << endl;
				}
				
			}
		}
	}
	return numOfTweets;
	inFile.close();
}

//Function to print tweets

int main()
{
	int option;
	int offset = 0;
	int numOfTweets = 0;


	string searchTerm, date;
	string line; //Get the line from the file

	ifstream inFile;
	//inFile.open("sampleTweets.csv"); //Open the csv file

	cout << "Welcome to this Data Driven Application." << endl;
	cout << "Please select an option:" << endl;
	cout << "" << endl;
	//Brief requirements
	cout << "1: Count all tweets." << endl;
	cout << "2: Count all tweets containing the word 'Money'" << endl;
	cout << "3: Count all tweets containing the word 'Politics'" << endl;
	cout << "4: Print out all tweets mentioning the word 'Paris'" << endl;
	cout << "5: Print out all tweets mentioning the word 'DreamWorks'" << endl;
	cout << "6: Print out all tweets mentioning the word 'Uber'" << endl;
	//Own queries
	cout << "7: Print out all tweets mentioning the word 'Cats'" << endl;
	cout << "8: Search by date." << endl;
	cout << "9: Search by word." << endl;
	cout << "10: Count all tweets mentionng the word 'Rabbit'" << endl;


	cin >> option;

	//Option 1
	if (option == 1)
	{
		if (inFile.good()) //If the file is found
		{
			cout << "Reading from file." << endl;
			while (!inFile.eof()) //If before the end of the file
			{
				numOfTweets++;
			}
			cout << "The total number of tweets is: " << numOfTweets << endl;
			system("Pause");
		}
	}

	//Option 2
	else if (option == 2)
	{
		//countTweets(inFile, numOfTweets, offset, "Money", "money", line);
		cout << "The total number of tweets containing the word 'Money' is: " << countTweets(inFile, numOfTweets, offset, "Money", "money", line) << endl;
		system("Pause");
	}

		/*if (inFile.good())
		{
			cout << "Reading from file." << endl;
			while (!inFile.eof())
			{
				while (getline(inFile, line))
				{
					if ((offset = line.find(" money ", 0)) != string::npos) //Place a space before and after the word to stop any potential words within words
						//This line finds the word is on the line and saves that line to the variable line.
					{
						numOfTweets++;
					}
					else if ((offset = line.find(" Money ", 0)) != string::npos)
						//Else if for case sensitivity. 
					{
						numOfTweets++;
					}

				}
				cout << "The total number of tweets containing the word 'Money' is: " << numOfTweets << endl;
				system("Pause");
			}
		}
	}*/

	//Option 3
	else if (option == 3)
	{
		if (inFile.good())
		{
			cout << "Reading from file." << endl;
			while (!inFile.eof())
			{
				while (getline(inFile, line))
				{
					if ((offset = line.find(" politics ", 0)) != string::npos)
					{
						numOfTweets++;
					}
					else if ((offset = line.find(" Politics ", 0)) != string::npos)
					{
						numOfTweets++;
					}
				}
				cout << "The total number of tweets containing the word 'Politics' is: " << numOfTweets << endl;
				system("Pause");
			}
		}
	}

	//Option 4
	else if (option == 4)
	{
		if (inFile.good())
		{
			cout << "Reading from file." << endl;
			while (!inFile.eof())
			{
				while (getline(inFile, line))
				{
					if ((offset = line.find(" Paris ", 0)) != string::npos)
					{
						numOfTweets++;
						cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl; //makes it look neater 
						cout << line << endl; //Print the tweet.
					}
					else if ((offset = line.find(" paris ", 0)) != string::npos)
					{
						numOfTweets++;
						cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
						cout << line << endl;
					}
				}
				cout << "The total number of tweets containing the word 'Paris' is: " << numOfTweets << endl;
				system("Pause");
			}
		}
	}

	//Option 5
	else if (option == 5)
	{
		if (inFile.good())
		{
			cout << "Reading from file." << endl;
			while (!inFile.eof())
			{
				while (getline(inFile, line))
				{
					if ((offset = line.find(" DreamWorks ", 0)) != string::npos)
					{
						numOfTweets++;
						cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
						cout << line << endl;
					}
					else if ((offset = line.find(" dreamworks ", 0)) != string::npos)
					{
						numOfTweets++;
						cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
						cout << line << endl;
					}
				}
				cout << "The total number of tweets containing the word 'DreamWorks' is: " << numOfTweets << endl;
				system("Pause");
			}
		}
	}

	//Option 6
	else if (option == 6)
	{
		if (inFile.good())
		{
			cout << "Reading from file." << endl;
			while (!inFile.eof())
			{
				while (getline(inFile, line))
				{
					if ((offset = line.find(" Uber ", 0)) != string::npos)
					{
						numOfTweets++;
						cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
						cout << line << endl;
					}
					else if ((offset = line.find(" uber ", 0)) != string::npos)
					{
						numOfTweets++;
						cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
						cout << line << endl;
					}
				}
				cout << "The total number of tweets containing the word 'Uber' is: " << numOfTweets << endl;
				system("Pause"); 
			}
		}
	}

	//Option 7
	else if (option == 7)
	{
		if (inFile.good())
		{
			cout << "Reading from file." << endl;
			while (!inFile.eof())
			{
				while (getline(inFile, line))
				{
					if ((offset = line.find(" Cats ", 0)) != string::npos)
					{
						numOfTweets++;
						cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
						cout << line << endl;
					}
					else if ((offset = line.find(" cats ", 0)) != string::npos)
					{
						numOfTweets++;
						cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
						cout << line << endl;
					}
				}
				cout << "The total number of tweets containing the word 'Cat' is: " << numOfTweets << endl;
				system("Pause");
			}
		}
	}

	//Option 8
	else if (option == 8)
	{

		if (inFile.good())
		{
			cout << "Please insert the date you want to search. (DD/MM/YYY)" << endl;
			cin >> date;
			cout << "Reading from file." << endl;
			while (!inFile.eof())
			{
				while (getline(inFile, line))
				{
					if ((offset = line.find(date, 0)) != string::npos)
					{
						numOfTweets++;
						cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
						cout << line << endl;

						if (numOfTweets == 0)
						{
							cout << "No Tweets Found." << endl;
						}
					}
				}
				cout << "The total number of tweets from " << date << " is: " << numOfTweets << endl;
				system("Pause");
			}
		}
	}
	else if (option == 9)
	{
		if (inFile.good())
		{
			cout << "Please insert the word you want to search." << endl;
			cin >> searchTerm;
			transform(searchTerm.begin(), searchTerm.end(), searchTerm.begin(), ::tolower); //Converts the inputted string to lowercase 
			cout << "Reading from file." << endl;
			while (getline(inFile, line))
			{
				if ((offset = line.find(searchTerm, 0)) != string::npos)
				{
					numOfTweets++;
					cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
					cout << line << endl;
				}
			}
			cout << "The total number of tweets containing " << searchTerm << " is: " << numOfTweets << endl;
			system("Pause");
		}
	}
	else if (option == 10)
	{
		if (inFile.good())
		{
			cout << "Reading from file." << endl;
			while (!inFile.eof())
			{
				while (getline(inFile, line))
				{
					if ((offset = line.find(" Rabbit ", 0)) != string::npos)
					{
						numOfTweets++;
						cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
						cout << line << endl;
					}
					else if ((offset = line.find(" rabbit ", 0)) != string::npos)
					{
						numOfTweets++;
						cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
						cout << line << endl;
					}
				}
				cout << "The total number of tweets containing the word 'Rabbit' is: " << numOfTweets << endl;
				system("Pause");
			}

		}
		else {
			cout << "No Option Entered!" << endl;
		}


		//Option 9


		system("Pause");
		return 0;
	}
}